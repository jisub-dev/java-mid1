...
# Section 11. 예외 처리2 - 실습
* 예외 객체를 만들어서 사용하는 메서드에 throws를 추가한다.(option + enter로 가능) 그 후 return을 void로 만든다. void로 만들면 메서드가 아무런 정보를 리턴하지 않는것 자체가 잘 동작한다는 것을 의미한다
* 오류 메세지를 의미있는걸로 만들고 잘 정리해놔야 나중에 편하다
* finally는 어떠한 경우라도 반드시 호출된다. try, catch 안에서 처리할 수 없는 예외가 발생해도 호출된다
* catch 없이 try, finally 만 사용할 수 있다
* 각 예외에 대한 클래스를 따로 만들어서 던질 수 있다
* catch할 때 디테일 한 예외를 앞에 두어야 한다. 만약 뒤에 나오는 예외보다 부모를 앞에 배치하면 앞에서 모두 예외처리가 되어 뒤에있는 구체적인 예외처리가 의미없기 때문이다
* 여러 예외를 잡을 때 catch (예외1 | 예외2) 처럼 or 연산자로 처리할 수 있다. 또한 각 예외들의 공통 부모의 기능만 사용할 수 있다
### 실무 예외 처리 방안
* 문제 제시
  1. 네트워크 서버 문제, 데이터베이스 서버 문제 -> 애플리케이션에서 연결 오류, 데이터베이스 접속 실패와 같은 예외 발생
  2. 이러한 시스템 오류로 인한 예외 중 해결할 수 없는 예외가 있음
  3. 이런 경우 "현재 시스템에 문제가 있습니다"와 같은 오류 메시지, 오류 페이지가 필요함
  4. 또한 개발자가 빠르게 인지할 수 있도록 로그를 남겨두어야 한다
* 체크 예외를 사용하면 서비스 아래에 네트워크, 데이터베이스 등 에서 감당할 수 없는 예외 처리가 필요할 수 있다
* 보통 서비스에서는 자신이 처리할 수 없는 예외가 많기 때문에 밖으로 던져야 한다 -> "고객님 죄송합니다"
* 그렇다고 throws Exception 으로 모든 예외를 던지면 (Exception은 모든 예외의 부모) 중간에 중요한 체크 예외도 다 못하게 되어 중대한 문제가 생길 수 있다
* 예외는 잡아서 복구할 수 있는 예외보다 복구할 수 없는 예외가 더 많다 -> 해결할 수 있는 예외만 잡아서 처리하는게 더 나을 수 있음
* 처리할 수 없는 예외는 던지고 모든 체크 예외를 명시해야한다
* 언체크 예외를 쓰면 throws를 선언하지 않아도 예외를 던진다
* 개발자가 해결할 수 없는 예외가 발생하면 공통 예외 처리로 묶어서 진행한다
* 예외를 복구 (예외를 catch함)하면 프로그램이 종료되지 않는다
* exceptionHandler()
  * 사용자에게 자세한 오류 상황을 보고하는건 좋지 않다
* try with resources: try 에서 외부 자원을 사용하고 반납하는 패턴을 간편하게 만들어 주는 것이다
  1. 먼저 AutoCloseable 인터페이스를 구현해야한다
  2. 그리고 try의 괄호 안에 AutoCloseable 인터페이스를 구현한 서비스 객체를 생성한다
  * 기존 코드 대비 스코프도 좁아진다 -> 디버깅 시 용이
  * 장점
    1. 리소스 누수 방지
    2. 코드 간결성 및 가독성 향상
    3. 스코프 범위 한정
